/**
 * Created by asus-200 on 2016/4/18.
 */
/**
 * Created by asus-200 on 2016/4/18.
 */
...
function setStyle3(obj, name, value) {
    obj.style["Webkit" + name.charAt(0).toUpperCase() + name.substring(1)] = value, obj.style["Moz" + name.charAt(0).toUpperCase() + name.substring(1)] = value, obj.style["ms" + name.charAt(0).toUpperCase() + name.substring(1)] = value, obj.style["O" + name.charAt(0).toUpperCase() + name.substring(1)] = value, obj.style[name] = value
}
...
function setStyle(obj, json) {
    if (obj.length)for (var i = 0; i < obj.length; i++)setStyle(obj[i], json); else for (var i in json)obj.style[i] = json[i]
}
function savePopShow(tips, position, or) {
    var len = arguments.length, oHide = ".save-success";
    ...
    $("body").append(saveHtml), $("#save-pop").find(oHide).hide().siblings("div").fadeIn(300).html(tips), $("#save-pop").css("position", "absolute").css("top", position.top + 5).css("left", position.left - 130).css("background", "#fafafa").fadeIn("slow").delay(600).fadeOut("slow")
}
...
Effect.browser_test = function () {
    IE6 = -1 != window.navigator.userAgent.search(/MSIE 6/), IE7 = -1 != window.navigator.userAgent.search(/MSIE 7/), IE8 = -1 != window.navigator.userAgent.search(/MSIE 8/), IE9 = -1 != window.navigator.userAgent.search(/MSIE 9/), IE10 = -1 != window.navigator.userAgent.search(/MSIE 10/)
}, Effect.flex = function (obj, cur, target, fnDo, fnEnd, fs, ms) {
    function fnMove() {
        obj.__flex_v += (100 - x) / fs, obj.__flex_v *= ms, Math.abs(obj.__flex_v) > MAX_SPEED && (obj.__flex_v = obj.__flex_v > 0 ? MAX_SPEED : -MAX_SPEED), x += obj.__flex_v;
        for (var i in cur)now[i] = (target[i] - cur[i]) * x / 100 + cur[i];
        fnDo && fnDo.call(obj, now), Math.abs(obj.__flex_v) < 1 && Math.abs(100 - x) < 1 && (clearInterval(obj.timer), fnEnd && fnEnd.call(obj, target), obj.__flex_v = 0)
    }

    if (Effect.browser_test.IE6)return fnDo && fnDo.call(obj, target), void(fnEnd && fnEnd.call(obj, target));
    var MAX_SPEED = 16;
    fs || (fs = 6), ms || (ms = .75);
    var now = {}, x = 0;
    obj.__flex_v || (obj.__flex_v = 0), obj.__last_timer || (obj.__last_timer = 0);
    var t = (new Date).getTime();
    t - obj.__last_timer > 20 && (fnMove(), obj.__last_timer = t), clearInterval(obj.timer), obj.timer = setInterval(fnMove, 20)
}, Effect.buffer = function (obj, cur, target, fnDo, fnEnd, fs) {
    function fnMove() {
        v = Math.ceil((100 - x) / fs), x += v;
        for (var i in cur)now[i] = (target[i] - cur[i]) * x / 100 + cur[i];
        fnDo && fnDo.call(obj, now), Math.abs(v) < 1 && Math.abs(100 - x) < 1 && (clearInterval(obj.timer), fnEnd && fnEnd.call(obj, target))
    }
    ...
    function fnMove() {
        x += v;
        for (var i in cur)now[i] = (target[i] - cur[i]) * x / 100 + cur[i];
        fnDo && fnDo.call(obj, now), Math.abs(100 - x) < 1 && (clearInterval(obj.timer), fnEnd && fnEnd.call(obj, target))
    }
    ...
    obj.__last_timer || (obj.__last_timer = 0);
    var t = (new Date).getTime();
    t - obj.__last_timer > 20 && (fnMove(), obj.__last_timer = t), clearInterval(obj.timer), obj.timer = setInterval(fnMove, 20), v = 100 / fs
};
var buffer = Effect.buffer, linear = Effect.linear, flex = Effect.flex;
$(function () {
    ...
    function pageTurn() {
        ...
        $bannerCenterA.eq(todo).addClass("current").siblings("a").removeClass("current"), $bannerImg.attr("href", galleryHref)
    }

    function resetCanvas() {
        context.clearRect(0, 0, 12, 12), context.fillStyle = "#39c", context.beginPath(), context.arc(6, 6, 6, 0, 2 * Math.PI), context.lineTo(6, 6), context.closePath(), context.fill()
    }

    function drawCircle() {
        Math.abs(dps - 1.5 * Math.PI) < 1e-6 && (clearInterval(intervalId), resetCanvas(), spinCanvas()), context.fillStyle = "#FFF", context.beginPath(), context.arc(6, 6, 6, -.5 * Math.PI, dps, !1), context.lineTo(6, 6), context.closePath(), context.fill(), dps += Math.PI / 180 * 4
    }

    function spinCanvas() {
        dps = -.5 * Math.PI + Math.PI / 180 * 4, clearInterval(intervalId), context.fillStyle = "#39c", context.beginPath(), context.arc(6, 6, 6, 0, 2 * Math.PI), context.closePath(), context.fill(), intervalId = setInterval(function () {
            drawCircle()
        }, 100)
    }
    ...
    var now = 0, oDiv = document.getElementById("banner_img"), ready = !0, W = 1080, H = 300, todo = (document.createElement("div"), 1), autoTime = 9e3, $bannerCenterA = $(".banner_center a"), $bannerImg = $("#banner_img"), $bannerPrev = $(".banner_prev"), $bannerNext = $(".banner_next"), bannerLen = $bannerCenterA.length;
    ...
    if ($("#banner").hover(function () {
        ...
        var canvas = document.getElementById("dotCanvas"), context = canvas.getContext("2d");
        context.fillStyle = "#39c", context.beginPath(), context.arc(6, 6, 6, 0, 2 * Math.PI), context.closePath(), context.fill();
        var dps = -.5 * Math.PI + Math.PI / 180 * 4, intervalId = setInterval(function () {
            drawCircle()
        }, 100);
        $("#banner").hover(function () {
            resetCanvas(), clearInterval(intervalId)
        }, function () {
            spinCanvas()
        })
    }
    var boomEffect = function (iNext) {
        if (ready) {
            ready = !1;
            var R = 4, C = 7, cw = W / 2, ch = H / 2;
            oDiv.innerHTML = "", oDiv.style.background = getBannerUrl(iNext) + " center no-repeat";
            for (var aData = [], wait = R * C, i = 0; R > i; i++)for (var j = 0, k = 0; C > j; j++, k++) {
                ...
                var l = (aData[i].left + W / (2 * C) - cw) * rnd(2, 3) + cw - W / (2 * C), t = (aData[i].top + H / (2 * R) - ch) * rnd(2, 3) + ch - H / (2 * R);
                setTimeout(function (oNewDiv, l, t) {
                    return function () {
                        buffer(oNewDiv, {
                            ...
                            this.style.left = now.left + "px", this.style.top = now.top + "px", this.style.opacity = now.opacity / 100, setStyle3(oNewDiv, "transform", "perspective(500px) rotateX(" + now.x + "deg) rotateY(" + now.y + "deg) rotateZ(" + now.z + "deg) scale(" + now.scale + ")")
                        }, function () {
                            setTimeout(function () {
                                $bannerImg.empty()
                            }, 200), 0 == --wait && (ready = !0, now = iNext)
                        }, 10)
                    }
        ...
        if (ready) {
            ready = !1;
            var R = 3, C = 6, wait = R * C, dw = Math.ceil(W / C), dh = Math.ceil(H / R);
            oDiv.style.background = "none", oDiv.innerHTML = "";
            for (var i = 0; C > i; i++)for (var j = 0; R > j; j++) {
                var oNewDiv = document.createElement("div"), t = Math.ceil(H * j / R), l = Math.ceil(W * i / C);
                setStyle(oNewDiv, {
                    position: "absolute",
                    ...
                    oNewDiv.ch = !1, setTimeout(function () {
                        linear(oNewDiv, {y: 0}, {y: 180}, function (now) {
                            now.y > 90 && !oNewDiv.ch && (oNewDiv.ch = !0, oNewDiv.style.background = getBannerUrl(iNext) + -l + "px " + -t + "px no-repeat"), now.y > 90 ? setStyle3(oNewDiv, "transform", "perspective(500px) rotateY(" + now.y + "deg) scale(-1,1)") : setStyle3(oNewDiv, "transform", "perspective(500px) rotateY(" + now.y + "deg)")
                        }, function () {
                            0 == --wait && (ready = !0, now = iNext)
                        }, 22)
                    }, 200 * (i + j))
                }(oNewDiv, l, t), oDiv.appendChild(oNewDiv)
            }
        }
    }, pageEffect = function (iNext) {
        if (ready) {
            ready = !1, oDiv.innerHTML = "", oDiv.style.background = getBannerUrl(iNext) + " center no-repeat";
            ...
            setStyle(oDivPage, {
                ...
                height: "100%",
                overflow: "hidden"
            }), setStyle3(oDivPage, "transform", "perspective(1000px) rotateY(0deg)"), setStyle3(oDivPage, "transformOrigin", "left"), oDiv.appendChild(oDivPage);
            ...
            setStyle(oDivShadow, {
            ...
            }), oDiv.appendChild(oDivShadow), oDivPage.ch = !1, buffer(oDivPage, {y: 0, opacity: 1}, {
                y: -180,
                opacity: 0
            }, function (now) {
                if (now.y < -90 && !oDivPage.ch) {
                    oDivPage.ch = !0, oDivPage.innerHTML = "<img />";
                    var oImg = oDivPage.getElementsByTagName("img")[0];
                    oImg.src = getBannerSrc(iNext), setStyle3(oImg, "transform", "scaleX(-1)"), setStyle(oImg, {
                        position: "absolute",
                        right: 0,
                        top: 0,
                        width: "200%",
                ...
                now.y < -90 ? setStyle3(oDivPage, "transform", "perspective(1000px) scale(-1,1) rotateY(" + (180 - now.y) + "deg)") : setStyle3(oDivPage, "transform", "perspective(1000px) rotateY(" + now.y + "deg)"), oDivShadow.style.background = "rgba(0,0,0," + now.opacity + ")"
            }, function () {
                setTimeout(function () {
                    $bannerImg.empty()
                }, 200), now = iNext, ready = !0
            }, 14)
        }
    }
}), function ($) {
    ...
    $.toJSON = "object" == typeof JSON && JSON.stringify ? JSON.stringify : function (o) {
        ...
        if ("object" == typeof o) {
            for (k in o)if (hasOwn.call(o, k)) {
                if (type = typeof k, "number" === type)name = '"' + k + '"'; else {
    ...
    var OnePageNav = function (elem, options) {
        this.elem = elem, this.$elem = $(elem), this.options = options, this.metadata = this.$elem.data("plugin-options"), this.$nav = this.$elem.find("a"), this.$win = $(window), this.sections = {}, this.didScroll = !1, this.$doc = $(document), this.docHeight = this.$doc.height()
    };
    OnePageNav.prototype = {
        defaults: {
            ...
            var self = this;
            return self.config = $.extend({}, self.defaults, self.options, self.metadata), "" !== self.config.filter && (self.$nav = self.$nav.filter(self.config.filter)), self.$nav.on("click.onePageNav", $.proxy(self.handleClick, self)), self.getPositions(), self.bindInterval(), self.$win.on("resize.onePageNav", $.proxy(self.getPositions, self)), this
        }, adjustNav: function (self, $parent) {
            self.$elem.find("." + self.config.currentClass).removeClass(self.config.currentClass), $parent.addClass(self.config.currentClass)
        }, bindInterval: function () {
            var docHeight, self = this;
            self.$win.on("scroll.onePageNav", function () {
                self.didScroll = !0
            }), self.t = setInterval(function () {
                docHeight = self.$doc.height(), self.didScroll && (self.didScroll = !1, self.scrollChange()), docHeight !== self.docHeight && (self.docHeight = docHeight, self.getPositions())
            }, 250)
            ...
            self.$nav.each(function () {
                linkHref = self.getHash($(this)), $target = $("#" + linkHref), $target.length && (topPos = $target.offset().top, self.sections[linkHref] = Math.round(topPos) - self.config.scrollOffset)
            })
    ...
    var k, v, y, d = "2.6.2", e = {}, f = !0, g = b.documentElement, h = "modernizr", i = b.createElement(h), j = i.style, m = ({}.toString, " -webkit- -moz- -o- -ms- ".split(" ")), n = "Webkit Moz O ms", o = n.split(" "), p = n.toLowerCase().split(" "), q = {}, t = [], u = t.slice, w = function (a, c, d, e) {
    ...
    $.PFold = function (options, element) {
        ...
        _init: function (options) {
            this.options = $.extend(!0, {}, $.PFold.defaults, options), this.transEndEventNames = {
                WebkitTransition: "webkitTransitionEnd",
                MozTransition: "transitionend",
                OTransition: "oTransitionEnd",
                msTransition: "MSTransitionEnd",
                transition: "transitionend"
            }, this.transEndEventName = this.transEndEventNames[Modernizr.prefixed("transition")], this.support = Modernizr.csstransitions && Modernizr.csstransforms3d, this.support && (this.$el.css("perspective", this.options.perspective + "px"), this.$el.css("transition", "all " + this.options.speed * this.options.folds * this.options.containerSpeedFactor + "ms " + this.options.containerEasing)), this.initialDim = {
                width: this.$el.width(),
                height: this.$el.height(),
                left: 0,
                top: 0
            }, this._layout(), this.$iContent = this.$el.find(".uc-initial"), this.iContent = this.$iContent.html(), this.$fContent = this.$el.find(".uc-final"), this.fContent = this.$fContent.html(), this.$finalEl = $('<div class="uc-final-wrapper"></div>').append(this.$iContent.clone().hide(), this.$fContent).hide(), this.$el.append(this.$finalEl), this._setDimOffset(), this.opened = !1, this.animating = !1, this._initEvents()
        }, _layout: function () {
            var $initialContentEl = this.$el.children("div.uc-initial-content"), finalDim = this._getFinalDim(), $finalContentEl = this.$el.children("div.uc-final-content").css({
                width: finalDim.width,
                height: finalDim.height
            });
            $initialContentEl.wrap('<div class="uc-initial"></div>'), $finalContentEl.show().wrap($('<div class="uc-final"></div>'))
            ...
            this.initialDim.offsetL = this.$el.offset().left - $window.scrollLeft(), this.initialDim.offsetT = this.$el.offset().top - $window.scrollTop(), this.initialDim.offsetR = $window.width() - this.initialDim.offsetL - this.initialDim.width, this.initialDim.offsetB = $window.height() - this.initialDim.offsetT - this.initialDim.height
        }, _getTranslationValue: function () {
            for (var x = 0, y = 0, horizTimes = 0, vertTimes = 0, i = 0; i < this.options.folds; ++i) {
                var dir = this.options.folddirection[i] || "bottom";
                switch (dir) {
                    case"left":
                        x += this.initialDim.width * Math.pow(2, horizTimes) / 2, horizTimes += 1;
                        break;
                    case"right":
                        x -= this.initialDim.width * Math.pow(2, horizTimes) / 2, horizTimes += 1;
                        break;
                    case"top":
                        y += this.initialDim.height * Math.pow(2, vertTimes) / 2, vertTimes += 1;
                        break;
                    case"bottom":
                        y -= this.initialDim.height * Math.pow(2, vertTimes) / 2, vertTimes += 1
                }
            ...
            for (var l = 0, r = 0, t = 0, b = 0, horizTimes = 0, vertTimes = 0, i = 0; i < this.options.folds; ++i) {
                var dir = this.options.folddirection[i] || "bottom";
                switch (dir) {
                    case"left":
                        l += this.initialDim.width * Math.pow(2, horizTimes), horizTimes += 1;
                        break;
                    case"right":
                        r += this.initialDim.width * Math.pow(2, horizTimes), horizTimes += 1;
                        break;
                    case"top":
                        t += this.initialDim.height * Math.pow(2, vertTimes), vertTimes += 1;
                        break;
                    case"bottom":
                        b += this.initialDim.height * Math.pow(2, vertTimes), vertTimes += 1
                }
            }
            return {l: l, r: r, t: t, b: b}
        }, _getFinalDim: function () {
            for (var horizTimes = 0, vertTimes = 0, i = 0; i < this.options.folds; ++i) {
                var dir = this.options.folddirection[i] || "bottom";
                switch (dir) {
                    case"left":
                    case"right":
            ...
            step |= 0;
            var self = this, styleCSS = "fold" === action ? {
                width: this.lastStyle.width,
                height: this.lastStyle.height,
                left: this.lastStyle.left,
                top: this.lastStyle.top
            } : this.initialDim, contentTopFront = "", contentBottom = "", contentTopBack = "", direction = "fold" === action ? this.options.folddirection[this.options.folds - 1 - step] || "bottom" : this.options.folddirection[step] || "bottom", nextdirection = "fold" === action ? this.options.folddirection[this.options.folds - 2 - step] || "bottom" : "";
            switch (this.$el.find("div.uc-part").remove(), step) {
                ...
                case this.options.folds - 1:
                    if ("fold" === action) {
                        if (step === this.options.folds - 1 && (styleCSS = this.initialDim, contentTopFront = this.iContent), 0 === step) {
                            this._setDimOffset(), this.$el.css({left: 0, top: 0});
                            var content = this._setLastStep(direction, styleCSS), contentBottom = content.bottom, contentTopBack = content.top;
                        ...
                        if (0 === step) {
                            ...
                            this.$el.addClass("uc-current").css({
                                left: coords.ftx,
                                top: coords.fty
                            }), contentTopFront = this.iContent, this.$finalEl.hide().children().hide()
                ...
                case this.options.folds:
                    ...
                    return this.$el.find(".uc-part").remove(), this.$finalEl.css(styleCSS).show().children().eq(contentIdx).show(), this.opened = "fold" === action ? !1 : !0, this.animating = !1, "fold" === action ? (this.$el.removeClass("uc-current"), this.options.onEndFolding()) : this.options.onEndUnfolding(), !1;
                default:
                    styleCSS = this._updateStepStyle(action)
            }
            this.support && (styleCSS.transition = "all " + this.options.speed + "ms " + this.options.easing);
            var unfoldClass = "uc-unfold-" + direction, topElClasses = "fold" === action ? "uc-unfold uc-part " + unfoldClass : "uc-part " + unfoldClass, $topEl = $('<div class="' + topElClasses + '"><div class="uc-front">' + contentTopFront + '</div><div class="uc-back">' + contentTopBack + "</div></div>").css(styleCSS), $bottomEl = $('<div class="uc-part uc-single">' + contentBottom + "</div>").css(styleCSS);
            this.lastDirection = "fold" === action ? nextdirection : direction, this.lastStyle = styleCSS, this.$el.append($bottomEl, $topEl), this.options.overlays && this.support && this._addOverlays(action, $bottomEl, $topEl), setTimeout(function () {
                if ("fold" === action ? $topEl.removeClass("uc-unfold") : $topEl.addClass("uc-unfold"), self.support ? $topEl.on(self.transEndEventName, function (event) {
                    ...
                    var bo = "fold" === action ? 1 : 0, tbo = "fold" === action ? .5 : 0, tfo = "fold" === action ? 0 : .5;
                    self.$bottomOverlay.css("opacity", bo), self.$topBackOverlay.css("opacity", tbo), self.$topFrontOverlay.css("opacity", tfo)
                }
            }, 30)
        }, _getTranslationViewport: function () {
            var accumulatedValues = this._getAccumulatedValue(), tx = 0, ty = 0;
            this.fOffsetL = this.initialDim.offsetL - accumulatedValues.l, this.fOffsetT = this.initialDim.offsetT - accumulatedValues.t, this.fOffsetR = this.initialDim.offsetR - accumulatedValues.r, this.fOffsetB = this.initialDim.offsetB - accumulatedValues.b, this.fOffsetL < 0 && (tx = Math.abs(this.fOffsetL)), this.fOffsetT < 0 && (ty = Math.abs(this.fOffsetT)), this.fOffsetR < 0 && (tx -= Math.abs(this.fOffsetR)), this.fOffsetB < 0 && (ty -= Math.abs(this.fOffsetB));
            var ftx = tx, fty = ty;
            if (this.options.centered) {
                var translationValue = this._getTranslationValue();
                ftx = translationValue.x > 0 && this.fOffsetR + translationValue.x >= 0 ? this.fOffsetL >= 0 ? Math.min(translationValue.x, this.fOffsetR) : translationValue.x + (tx - translationValue.x) : translationValue.x < 0 && this.fOffsetL + translationValue.x >= 0 && this.fOffsetR >= 0 ? Math.min(translationValue.x, this.fOffsetL) : translationValue.x + (tx - translationValue.x), fty = translationValue.y > 0 && this.fOffsetB + translationValue.y >= 0 ? this.fOffsetT >= 0 ? Math.min(translationValue.y, this.fOffsetB) : translationValue.y + (ty - translationValue.y) : translationValue.y < 0 && this.fOffsetT + translationValue.y >= 0 && this.fOffsetB >= 0 ? Math.min(translationValue.y, this.fOffsetT) : translationValue.y + (ty - translationValue.y)
            }
            return {ftx: ftx, fty: fty}
        }, _setLastStep: function (direction, styleCSS) {
            var contentBottom, contentTopBack, contentBottomStyle = "", contentTopBackStyle = "";
            switch (direction) {
                case"bottom":
                ...
                case"top":
                    ...
                    break;
                case"left":
                    contentTopBackStyle = "width:" + 2 * styleCSS.width + "px", contentBottomStyle = "width:" + 2 * styleCSS.width + "px;margin-left: -" + styleCSS.width + "px";
                    break;
                case"right":
                    contentTopBackStyle = "with:" + 2 * styleCSS.width + "px;margin-left: -" + styleCSS.width + "px", contentBottomStyle = "width:" + 2 * styleCSS.width + "px"
            }
            ...
            this.$bottomOverlay = $('<div class="uc-overlay"></div>'), this.$topFrontOverlay = $('<div class="uc-flipoverlay"></div>'), this.$topBackOverlay = $('<div class="uc-flipoverlay"></div>'), "fold" === action ? (bottomOverlayStyle = {transition: "opacity " + this.options.speed / 2 + "ms " + this.options.easing + " " + this.options.speed / 2 + "ms"}, topFrontOverlayStyle = {
                opacity: .5,
                ...
                opacity: 1,
                transition: "opacity " + this.options.speed / 2 + "ms " + this.options.easing
            }, topFrontOverlayStyle = {transition: "opacity " + this.options.speed / 2 + "ms " + this.options.easing}, topBackOverlayStyle = {
                opacity: .5,
                transition: "opacity " + this.options.speed / 2 + "ms " + this.options.easing + " " + this.options.speed / 2 + "ms"
            }), $bottomEl.append(this.$bottomOverlay.css(bottomOverlayStyle)), $topEl.children("div.uc-front").append(this.$topFrontOverlay.css(topFrontOverlayStyle)).end().children("div.uc-back").append(this.$topBackOverlay.css(topBackOverlayStyle))
        }, unfold: function () {
    ...
    function addPiano() {
        $(".menu li a").after('<div class="hover-bg"></div>');
        {
            ...
            setTimeout(function () {
                ...
                $(".piano_audio").append(audio_html), piano.audioOn = 1
            }, 1e3)
        }
    }
    ...
    function initAutoMusic(initUrl) {
        var testTimer = setInterval(function () {
            (piano.audioOn = 1) && (clearInterval(testTimer), setTimeout(function () {
                var paramReg = /\/\#music=/, start = initUrl.search(paramReg) + 8, end = initUrl.length, params = initUrl.substring(start, end), dataArr = params.split("-"), music = parseData(dataArr);
                listen(music)
            }, 1500))
        }, 500)
    }
    ...
    function checkAudioCompat() {
        var myAudio = document.createElement("audio"), msg = "";
    ...
    function timeRun() {
        piano.step = 0, clearInterval(piano.timeRuner), piano.timeRuner = setInterval(function () {
            piano.step += 100, piano.timeCount = piano.step
        }, 100)
    }
    ...
    function startRecord(key) {
        if (status.play)if (status.end = !0, "" != piano.url)if (0 != key) {
            ...
            piano.url += "-" + key + ":" + urlTime, timeRun()
        } else piano.url += "-" + key + ":0"; else piano.url += key + ":0", timeRun()
    }

    function playPiano(i) {
        var key = i - 1;
        piano.keyDom.eq(key).children("div").stop(!0, !1).animate({top: "0px"}, 200), piano.keyDom.eq(key).children("div").animate({top: "66px"}, 200), playKey(key)
    }

    function share() {
        var shareCourseTitle = $(".share_title").data("title"), sinaTitle = "分享节奏 「" + shareCourseTitle + "」 -轩枫阁（分享自@轩枫Y_me）", renrenTitle = "分享节奏 「" + shareCourseTitle + "」-轩枫阁（分享自@农航亮(356948827)）", tqqTitle = "分享节奏 「" + shareCourseTitle + "」-轩枫阁（分享自@轩枫阁）";
        ...
        shareUrl = encodeURIComponent($(".share_record").attr("href")), $("body").pianoSnsShare({
            tsina: {
                url: shareUrl,
                title: sinaTitle,
                pic: picShare
            },
            renren: {srcUrl: shareUrl, title: renrenTitle, pic: picShare},
            tqq: {url: shareUrl, title: tqqTitle, pic: picShare},
            tqzone: {url: shareUrl, title: tqzoneTitle, pic: picShare}
        })
    }

    function listen(playData) {
        for (var data = playData.data, len = data.length, startTime = 0, i = 0; len > i; i++) {
            var now = data[i], key = now.key, time = startTime + 100 * now.time;
            timeOut(key, time), startTime = time
        }
    }
    ...
    function analysis(list) {
        var testReg = /\/\#music=(\d:\d-?)*/, test = testReg.test(list);
        if (test) {
            var paramReg = /\/\#music=/, start = list.search(paramReg) + 8, end = list.length, params = list.substring(start, end), dataArr = params.split("-");
    ...
    function parseData(arr) {
        var autoPlayData = {};
        autoPlayData.data = [];
        for (var jsonObj = autoPlayData.data, len = arr.length, i = 0; len > i; i++) {
            var newArr = arr[i].split(":"), key = newArr[0], time = newArr[1];
            if (0 != key) {
                var newObj = {};
                newObj.key = key, newObj.time = time, jsonObj.push(newObj)
            }
        }
        return autoPlayData
    }
    ...
    function renderList(list) {
        ...
        tmp += '<li class="mysong"><span class="split">•</span><a href="http://www.xuanfengge.com/piano" data-title="' + i + '">我要提交曲谱>></a></li>', tmp += "</ul>", $("#list-info")[0].innerHTML = tmp
    }

    function renderSong(title) {
        var opern = $("#song-opern"), song = list[title];
        opern.find(".title").html(title);
        var arr = song.split("|"), keys = arr && arr[0], from = $("#song-opern").find(".opern-from");
        if (from.hide(), keys.length) {
            var COUNT = 35, len = Math.ceil(keys.length / COUNT);
            $("#opern-info")[0].innerHTML = "";
            for (var i = 0; len > i; i++) {
                var l = COUNT * (i + 1), tmp1 = tmp2 = "";
                ...
                $("#opern-info")[0].innerHTML += '<dl class="number"><dt>乐谱</dt>' + tmp1 + "</dl>", $("#opern-info")[0].innerHTML += '<dl class="letter"><dt>键盘弹奏</dt>' + tmp2 + "</dl>"
            }
        }
        if (arr[1]) {
            var link = from.find("a");
            link.html(arr[1]), arr[2] && link.attr("href", arr[2]), from.show()
        }
    }

    function showSongList() {
        $("#song-opern").hide(), $("#song-list").show()
    }
    ...
    if ($(".music_trangle").hover(function () {
            clearTimeout(piano.extraTimer), $(".piano_extra").show(), $(this).addClass("trangle_hover")
        }, function () {
            piano.extraTimer = setTimeout(function () {
                $(".piano_extra").hide(), $(".music_trangle").removeClass("trangle_hover")
            }, 800)
        }), $(".piano_extra").hover(function () {
            clearTimeout(piano.extraTimer), $(this).show(), $(".music_trangle").addClass("trangle_hover")
        }, function () {
            $(this).hide(), $(".music_trangle").removeClass("trangle_hover")
        }), $(".book_play").click(function () {
            renderList(list), $(".piano_album").show(), $(".album_choose").show(), $(".piano_function").hide(), $(".song-list").show(), $(".song-opern").hide(), $(".save_extra").hide()
        }), $(".song_record").click(function () {
            $(".album_choose").hide(), $(".piano_function").show(), $(".piano_album").show(), $(".record-area").show(), $(".record-success").hide()
        }), $(".piano_extra").click(function () {
            $("#banner, .feature, #top, #logo").hide(), $("#nav").animate({width: "980px"}), $("#nav li").animate({width: "108.5px"})
        }), $(".piano_album").on("click", ".album_close", function () {
            $("#banner, .feature, #top, #logo").show(), $("#nav li").css({width: "90px"}), $("#nav").css({width: "810px"})
        }), $.browser.msie && $.browser.version < 9) {
        $(".menu li a").after('<div class="hover-bg"></div>'), $(".menu").addClass("menupiano"), $(".music_extra_nav").hide();
        var testReg = /\/\#music=(\d:\d-?)*/, test = testReg.test(location.href);
        if (test) {
            {
                $("#logo").offset()
            }
    ...
    piano.keyDom.each(function (i) {
        $(this).hover(function () {
            $(this).children("div").stop(!0, !1).animate({top: "0px"}, 200), playKey(i), startRecord(i + 1)
        }, function () {
    ...
    var status = {play: !1, copy: !1, end: !1, reset: !1};
    $(".piano_function").on("click", ".start_record", function () {
        status.play = !0, $(".cancel_record").show(), $(".finish_record").show(), $(".record-state").text("正在录制，请使用鼠标或键盘弹奏"), $(this).hide()
    }), $(".piano_function").on("click", ".cancel_record", function () {
        status.play = !1, piano.url = "", status.end = !1, $(".finish_record").hide(), $(".start_record").show(), $(this).hide(), $(".record-state").text("点击开始录制，可以把你弹奏的曲子录制下来")
    }), $(".piano_function").on("click", ".finish_record", function () {
        if (status.end) {
            status.play = !1, status.end = !1;
            var allUrl = "http://www.xuanfengge.com/#music=" + piano.url;
            $(".share_record").attr("href", allUrl), piano.url = "";
            var copyCon = $(".share_record").attr("href"), flashvars = {
            ...
            swfobject.embedSWF("http://www.xuanfengge.com/wp-content/themes/lee2.0/js/clipboard.swf", "forLoadSwf", "120", "25", "9.0.0", null, flashvars, params), shareUrl = encodeURIComponent($(".share_record").attr("href")), share(), $(".record-success .record-info").hide(), $(".start_listen, .save_record").show(), $(".record-share").show(), $(".record-success .record-panel").show()
        } else $(".record-success .record-info").show(), $(".start_listen, .save_record").hide(), $(".upload_record").hide(), $(".record-share").hide();
        $(".record-success").show(), $(".record-state").text("点击开始录制，可以把你弹奏的曲子录制下来"), $(".start_record").show(), $(".finish_record, .cancel_record").hide(), $(".record-area").hide()
    }), $(".piano_function").on("click", ".start_listen", function () {
        var shareLink = $(".share_record").attr("href"), music = analysis(shareLink);
        listen(music)
    }), $(".piano_function").on("click", ".reset_record", function () {
        $(".record-area").show(), $(".record-success").hide()
        ...
        status.play = !1, status.end = !1, $(".finish_record").hide(), $(".start_record").show(), $(".cancel_record").hide(), $(".record-state").text("点击开始录制，可以把你弹奏的曲子录制下来"), $(".piano_function").hide(), $(".record-success").hide()
    }), $(".album_choose").on("click", ".song a", function () {
        $(".piano_function").show()
    });
    var shareUrl = "";
    $(".piano_album").on("click", ".album_close", function () {
        status.play = !1, piano.url = "", status.end = !1, $(".finish_record").hide(), $(".start_record").show(), $(".record-state").text("点击开始录制，可以把你弹奏的曲子录制下来"), $(".piano_album").hide(), $(".song-list").show(), $(".song-opern").hide(), $(".record-success").hide(), $(".record-area").show()
    });
    var list = {
    ...
    }, map = {1: "A", 2: "S", 3: "D", 4: "F", 5: "G", 6: "H", 7: "J", 8: "K", 9: "L"};
    $("#list-info").delegate(".song a", "click", function (e) {
        ...
        renderSong(title), $("#song-list").hide(), $("#song-opern").show(), $(".record-area").show(), $(".record-success").hide()
    }), $("#song-opern").delegate(".select-another", "click", function (e) {
        e.preventDefault(), showSongList()
    }), $(".save_listen").click(function () {
        var shareLink = $(this).data("list"), music = analysis(shareLink);
        listen(music)
    ...
    $.fn.pianoSnsShare = function (options) {
        ...
        }), settings = $.extend(!0, {}, defaults, options), shareIconName = {
            tsina: "新浪微博",
            renren: "人人网",
            tqq: "腾讯微博",
            tqzone: "QQ空间"
...
var UserData = {
    userData: null, name: location.href, init: function () {
        if (!UserData.userData)try {
            UserData.userData = document.createElement("INPUT"), UserData.userData.type = "hidden", UserData.userData.style.display = "none", UserData.userData.addBehavior("#default#userData"), document.body.appendChild(UserData.userData);
            var expires = new Date;
            expires.setDate(expires.getDate() + 365), UserData.userData.expires = expires.toUTCString()
        } catch (e) {
            return !1
        }
        return !0
...
}, storage = getLocalStorage(), LocalStorage = {
    setItem: function (key, value) {
...
$(function () {
    function getDataText() {
        var key = getKey(), title = $(".save_info").val().trim(), link = $(".share_record").attr("href"), book = selectBook(link), data = {
            key: key,
            title: title,
            link: link,
            book: book
    ...
    function insertData() {
        var indexKey = "music_" + getKey(), dataKey = LocalStorage.getItem(indexKey), musicData = getDataText(), position = $(".save_info").offset();
    ...
    function selectBook(link) {
        var paramReg = /\/\?music=/, start = link.search(paramReg) + 8, end = link.length, params = link.substring(start, end), result = params.match(/\d{1}:/g).join("").replace(/:/g, "");
    ...
    function dataRead() {
        for (var result = [], value = "", i = 0, len = storage.length; len > i; i++) {
            var key = storage.key(i), reg = /music_\d*/g;
    ...
    function isEmpty() {
        for (var i = 0, len = storage.length; len > i; i++) {
            var key = storage.key(i), reg = /music_\d*/g;
    ...
    function view(data) {
        for (var len = data.length, i = 0; len > i; i++) {
            var oData = $.parseJSON(data[i]), key = oData.key, title = oData.title, link = oData.link, book = oData.book;
            albumList[key] = {}, albumList[key].title = title, albumList[key].link = link, albumList[key].book = book
        }
        return albumList
    }
    ...
    function renderList(list) {
        ...
        tmp += '<li class="mysong"><span class="split">•</span><a href="http://www.xuanfengge.com/piano" target="_blank" data-title="' + i + '">我要提交曲谱>></a></li>', tmp += "</ul>", $("#list-info")[0].innerHTML = tmp
    }

    function renderSong(key) {
        var opern = $("#song-opern"), song = albumList[key], title = song.title, keys = song.book, from = $("#song-opern").find(".opern-from");
        ...
        $(".save_listen").data("list", song.link), $(".share_record").attr("href", song.link), $(".save_extra").show(), $("#opern-info").data("key", key)
    }
    ...
    $(".my_album").on("click", function () {
        ...
        var empty = isEmpty();
        empty ? ($("#list-info").empty(), LocalStorage.setItem("musicKey", "0"), $(".song-list h3").text("还没有保存过任何曲子，请先去弹奏保存吧")) : (renderList(saveList), $(".song-list h3").text("请选择要弹奏的曲谱")), $(".piano_album").show(), $(".album_choose").show(), $(".piano_function").hide(), $(".song-list").show(), $(".song-opern").hide()
    }), $(".save_delete").click(function () {
        var key = "music_" + $("#opern-info").data("key"), dataKey = LocalStorage.getItem(key), position = $(".save_listen").offset();
        null == dataKey || "" == dataKey ? savePopShow("删除失败", position, "error") : (LocalStorage.removeItem(key), savePopShow("删除成功", position), albumList = {}), $(".my_album").click()
    }), $(".save_record").click(function () {
        var title_info = $(".album_choose:visible").find(".song-opern:visible").find(".title").text();
        "" != title_info && $(".save_info").val(title_info), $(".save_info, .save_record_btn").show()
    }), $(".save_record_btn").click(function () {
        var title = $(".save_info").val().trim();
        if ("" == title) {
            var position = $(".save_info").offset();
            savePopShow("内容为空！", position, "error")
        } else insertData(), $(".save_info, .save_record_btn").hide()
    }), $(".save_info").on("keydown", function (e) {
        e.stopPropagation()
    });
    var map = {1: "A", 2: "S", 3: "D", 4: "F", 5: "G", 6: "H", 7: "J", 8: "K", 9: "L"}, shareUrl = "";
    $("#list-info").delegate(".save_song a", "click", function (e) {
        ...
        renderSong(key), share(), $("#song-list").hide(), $("#song-opern").show(), $(".piano_function").show(), $(".record-area").show(), $(".record-success .record-panel").hide(), $(".record-share").show(), $(".record-success").show()
    })
});
...
$(function () {
    ...
    function dataRead() {
        for (var result = "", value = "", i = 0, len = storage.length; len > i; i++) {
            var key = storage.key(i), reg = /book_\d*/g;
    ...
    function view(data) {
        {
            ...
            oData.title, oData.url, oData.post_time, oData.save_time, oData.meta, oData.imgUrl, oData.comment, oData.view
        }
    }

    function clear() {
        for (var key, reg = /book_\d+/, i = 0, len = storage.length; len > i; i++)key = storage.key(i), reg.test(key) ? (console.log("succe: " + key), LocalStorage.removeItem(key), i -= 1, len -= 1) : console.log("error: " + key);
    ...
    {
        var title = $(".title").html(), url = $(".url").html(), post_time = $(".post_time").html(), save_time = $(".save_time").html(), meta = $(".meta").html(), imgUrl = $(".imgUrl").html(), comment = $(".comment").html(), view = $(".view").html(), data = {
            title: title,
            url: url,
            post_time: post_time,
            save_time: save_time,
    ...
    $("#data").html(dataRead()), $("#safghve").click(function () {
        var indexKey = "book_" + getKey(), dataKey = LocalStorage.getItem(indexKey);
